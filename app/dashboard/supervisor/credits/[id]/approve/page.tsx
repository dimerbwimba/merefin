...
export default async function ApproveCreditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function isValidMetadata(data: any): data is { purpose?: string; duration?: number } {
    return (
      data === null ||
      (typeof data === 'object' &&
  ...
  return (
    <div className="container mx-auto py-8 px-20">
      <h1 className="text-2xl text-center font-bold mb-6">Approuver la demande de crédit</h1>
      <CreditApprovalForm credit={safeCredit} action="approve" />
    </div>
  )
}
